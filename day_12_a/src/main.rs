...
fn solve(counter: &mut i32, mut history: Vec<String>, rules: &HashMap<String, HashSet<String>>) {
            ...
            if next == "end" {
                ...
                *counter += 1;
                    ...
                    || next.chars().next().unwrap().is_ascii_uppercase()
                {
                    ...
                    solve(counter, new_history, rules);
                }
            }
        }
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    solve(&mut counter, vec!["start".to_owned()], &rules);
